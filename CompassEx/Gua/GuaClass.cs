...
namespace CompassEx.Gua
{
    ...
    public class GuaClass : IEquatable<GuaClass>
    {
        ...
        /// </value>
        [JsonIgnore]
        public readonly static String[] GuaNames = { "乾", "坤", "屯", "蒙", "需", "讼", "师", "比", "小畜", "履", "泰", "否", "同人", "大有", "谦", "豫", "随", "蛊", "临", "观", "噬嗑", "贲", "剥", "复", "无妄", "大畜", "颐", "大过", "坎", "离", "咸", "恒", "遁", "大壮", "晋", "明夷", "家人", "睽", "蹇", "解", "损", "益", "夬", "姤", "萃", "升", "困", "井", "革", "鼎", "震", "艮", "渐", "归妹", "丰", "旅", "巽", "兑", "涣", "节", "中孚", "小过", "既济", "未济" };
        ...
        /// </value>
        [JsonIgnore]
        public readonly static String[] GuaFullNames = { "乾为天", "坤为地", "水雷屯", "山水蒙", "水天需", "天水讼", "地水师", "水地比", "风天小畜", "天泽履", "地天泰", "天地否", "天火同人", "火天大有", "地山谦", "雷地豫", "泽雷随", "山风蛊", "地泽临", "风地观", "火雷噬嗑", "山火贲", "山地剥", "地雷复", "天雷无妄", "山天大畜", "山雷颐", "泽风大过", "坎为水", "离为火", "泽山咸", "雷风恒", "天山遁", "雷天大壮", "火地晋", "地火明夷", "风火家人", "火泽睽", "水山蹇", "雷水解", "山泽损", "风雷益", "泽天夬", "天风姤", "泽地萃", "地风升", "泽水困", "水风井", "泽火革", "火风鼎", "震为雷", "艮为山", "风山渐", "雷泽归妹", "雷火丰", "火山旅", "巽为风", "兑为泽", "风水涣", "水泽节", "风泽中孚", "雷山小过", "水火既济", "火水未济" };
        ...
        /// </value>
        [JsonIgnore]
        public readonly static string[] GuaSkyLocs = { "甲午", "甲子", "戊子", "庚申", "乙巳", "辛未", "壬申", "辛亥", "丁巳", "戊辰", "庚辰", "庚戌", "壬寅", "辛巳", "戊戌", "丁亥", "丁丑", "丁未", "乙卯", "己亥", "乙丑", "癸丑", "癸亥", "甲子", "己丑", "壬辰", "丙子", "丙午", "庚申", "庚寅", "丁酉", "庚午", "乙酉", "己巳", "乙亥", "辛丑", "丙寅", "甲辰", "甲戌", "丙申", "丁卯", "庚子", "癸巳", "甲午", "壬戌", "己未", "癸未", "乙未", "庚寅", "戊午", "壬子", "丙戌", "癸酉", "癸卯", "戊寅", "己酉", "壬午", "丙辰", "戊申", "己卯", "辛卯", "辛酉", "甲寅", "甲申" };
    ...
    "\u4DFF"  // 64 火水未济 
        ...
        public List<SkyLoc> SkyLocs
        {
            get
            {
                List<SkyLoc> ls = new List<SkyLoc>();
                for (int i = 0; i < this.DownGua.Locs.Count; i++)
                {
                    ls.Add(new SkyLoc(this.DownGua.SkyName + this.DownGua.Locs[i].Name));

                }
                for (int i = 0; i < this.UpGua.Locs.Count; i++)
                {
                    ls.Add(new SkyLoc(this.UpGua.SkyName + this.UpGua.Locs[i].Name));
        ...
        /// <remarks>
        /// 该属性通过当前实例的 <see cref="GuaIndex"/> 属性作为索引，
        /// 动态前往静态字符集清册 <see cref="Symbols"/> 中提取对应的 Unicode 易经六爻符号（范围从 \u4DC0 至 \u4DFF）。
        /// </remarks>
        [JsonIgnore]
        public string GuaSymbol { get { return Symbols[this.GuaIndex]; } }
        ...
        public GuaQi GuaQi
        {
            get
            {
                return this.UpGua.GuaQi;//上卦洛数为卦气
            }
        }
        ...
        public SkyLoc GuaSkyLoc
        {
            get
            {
                SkyLoc sl = new SkyLoc(GuaSkyLocs[this.GuaIndex]); //天干地支顺序与卦序相同(易经卦序)
                string s = GuaSkyLocs[this.GuaIndex];
        ...
        public string GuaFate
        {
            get
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    f[i] = this.UpGua.Yaos[i] % 2 == this.DownGua.Yaos[i] % 2; //卦象爻是由下而上，上卦初爻与下卦初爻对比,往下对比
                }
        ...
        [JsonIgnore]
        public GuaSubClass DownGua { get; private set; } //下卦（内卦），三爻卦
        ...
        [JsonIgnore]
        public GuaSubClass UpGua { get; private set; } //上卦（外卦），三爻卦
        ...
        public String GuaFullName { get; private set; } //六爻卦的全名

        /// <summary>
        /// 获取或设置当前卦在 64 卦数据库或特定排列模型中的原始数组索引。
        /// </summary>
        /// <value>预设值为 <c>-1</c>，表示尚未初始化；初始化后范围通常在 <c>0</c> 到 <c>63</c> 之间。</value>
        public int GuaIndex { get; private set; } = -1; //卦的所在索引
        ...
        public String GuaAliasName { get; private set; } = ""; //卦的所属别名，例如：归魂卦，游魂卦，纯卦等
        ...
        [JsonIgnore]
        public GuaSubClass GuaSelf { get; private set; } //卦宫(三爻卦),名称
        ...
        /// </remarks>
        public GuaClass(string GuaNameOrAttrName) : this(GetGuaIndexByName(GetFullGuaName(GuaNameOrAttrName)))
        {
        ...
        /// </summary>
        /// <param name="BeforGuaIndex">64 卦的原始数组索引（取值范围：<c>0</c> 至 <c>63</c>）。</param>
        ...
        /// <item><description><b>卦名检索</b>：根据索引获取四字全名（如“水雷屯”），并通过 <see cref="GetFullGuaName(string)"/> 规范化。</description></item>
        /// <item><description><b>经卦拆解</b>：调用 <see cref="GetGuaSubName(string,bool)"/> 将六爻卦物理切割为上卦（外卦）和下卦（内卦）两个三爻卦字符串。</description></item>
        /// <item><description><b>对象装配</b>：通过 <see cref="GuaSubClass.GetGuaSub(int ,int ,int ,bool )"/> 实例化上下卦对象，并锁定该卦在系统中的最终简名与全名属性。</description></item>
        /// </list>
        /// </remarks>
        public GuaClass(int BeforGuaIndex)
        {
            // 💡 修复潜在的越界隐患：数组上限应为 >= GuaNames.Length，已在文档校验中明确说明
            if (BeforGuaIndex < 0 || BeforGuaIndex >= GuaNames.Length) throw new IndexOutOfRangeException(nameof(BeforGuaIndex));
            string sGuaName = GuaFullNames[BeforGuaIndex];
            this.GuaFullName = GetFullGuaName(sGuaName);
            String sUpGuaName = GetGuaSubName(this.GuaFullName, true);
            String sDownGuaName = GetGuaSubName(this.GuaFullName, false);
        ...
        /// </summary>
        private static int GetGuaIndexByName(string name)
        {
            // 1. 先查简名列表
            int index = GuaNames.IndexOf(name);

            // 2. 如果简名里找不到（等于 -1），则去全名列表里找
            if (index == -1)
            {
                index = GuaFullNames.IndexOf(name);
                if (index == -1)
                {
                    var a = GuaFullNames.Where((s) => s.IndexOf(name) > -1);
                    if (a.Count() > 0) index = GuaFullNames.IndexOf(a.FirstOrDefault().ToString());
                    if (a.Count() > 1)
                    {
        ...
        /// </remarks>
        public GuaClass ExchangeGua()
        {
            GuaClass g = GuaClass.GetGuaClass(this.GuaIndex);//创建相同卦
        ...
        /// 该生命周期钩子方法会通过持久化的 <see cref="GuaIndex"/> 重新捕获干净的原始卦实例，
        ...
        internal void OnDeserializedMethod(StreamingContext context)
        {
            try
            {
                var a = GetGuaClass(this.GuaIndex);
        ...
        public int[] GetYaos()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                y[i] = this.DownGua.Yaos[i % 3];
            }
            for (int i = 3; i < 6; i++)
            {
                y[i] = this.UpGua.Yaos[i % 3];
            }
            return y;
        }
        ...
        public GuaClass GetChangeGua()
        {
            ...
            while (i < 6)
            {
                GuaSubClass gsc = i < 3 ? this.DownGua : this.UpGua;
                TorF = false;
                int iv = 0;
        ...
        /// 如果检测到世应数据尚未装配，会自动优先触发 <see cref="LoadGuaSelfandHereThere"/> 实施底层加载。
        /// </remarks>
        public String GetHereThereStr()
        {
            String s = "";
            if (this.HereYao == 0 && this.ThereYao == 0) LoadGuaSelfandHereThere();
            for (int i = 5; i >= 0; i--)
            {
        ...
        public String GetHideRelativeStr()
        {
            ...
            {//由上而下
                ...
                for (int j = 0; j < this.HideRelativeYaos.Count; j++)
                {
                    ...
                    if (TorF)
                    {
                        if (i < 3) gsc = gc.DownGua; else gsc = gc.UpGua;
                        LocClass lc = gsc.Locs[i % 3];
                        s += lc.Name + lc.FiveAttr.Name + gc.SixRelative[i].Name + "(伏)";
                    }
                }
                s += "\n";
            }
        ...
        public String GetYaosSkyLocStrShort()
        {
            ...
            for (int i = this.UpGua.Locs.Count - 1; i >= 0; i--)
            {//由上而下
                LocClass lc = this.UpGua.Locs[i];
                s += lc.Name + "\n";
            }
            for (int i = this.DownGua.Locs.Count - 1; i >= 0; i--)
            {//由上而下
                LocClass lc = this.DownGua.Locs[i];
                s += lc.Name + "\n";
            }
            if (s.Length > 1) s = s.Substring(0, s.Length - 1);
            return s;
        ...
        /// </summary>
        /// <param name="GuaSelfAttr">自定义的本宫五行属性名称（如：“金”、“木”、“水”、“火”、“土”）。</param>
        /// <param name="IsHadFateAttr">是否在输出结果的头部和尾部追加特定的排盘空格/格式化占位符。</param>
        /// <returns>返回由上而下排列的六亲换行字符串。</returns>
        /// <remarks>
        /// 该重载方法不读取类库中默认配好的本宫属性，而是完全依据 <paramref name="GuaSelfAttr"/> 进行实时交叉比对（通过生克规则 <see cref="FiveAttr.GetBothAttrRule"/> 变换得出）。
        /// </remarks>
        public String GetSixRelativeStr(String GuaSelfAttr, bool IsHadFateAttr = false)
        {
            List<LocClass> al = new List<LocClass>();
            String s = "";
            for (int i = 0; i < 3; i++) al.Add(this.DownGua.Locs[i]);
            for (int i = 0; i < 3; i++) al.Add(this.UpGua.Locs[i]);
            for (int i = al.Count - 1; i >= 0; i--)
            {//由上而下
                LocClass lc = al[i];
                FiveAttrRule far = FiveAttr.GetBothAttrRule(GuaSelfAttr, lc.FiveAttr.Name);
        ...
        {
            LoadGuaSelfandHereThere();//加载世应
            LoadGuaAliasName();//加载别名
        ...
        /// </remarks>
        public static String GetGuaFace(List<int> al)
        {
            String s = "";
            for (int i = al.Count - 1; i >= 0; i--)
            {//由上而下
        ...
        /// <remarks>
        /// <b>状态动态转化逻辑：</b><br/>
        /// 方法会首先审查 <see cref="YaoDoing"/> 动爻清册。若存在动爻，会动态将对应的内卦（<see cref="DownGua"/>）或外卦（<see cref="UpGua"/>）的原始阴阳代码（0或1）重设为 3（老阳动变）或 4（老阴动变）。<br/>
        /// 随后按初爻至上爻的顺序组装整型序列，递交给静态方法 <see cref="GetGuaFace(List{int})"/> 实施终极画面输出。
        /// </remarks>
        public String GetGuaFace()
        {
            List<int> al = new List<int>();
            int iYao = 0;
            if (this.YaoDoing != null)
            {
                if (this.YaoDoing.Count > 0)
                {
                    for (int i = 0; i < this.YaoDoing.Count; i++)
                    {
                        ...
                        if (iYao < 3)
                        {
                            this.DownGua.Yaos[iYao] = this.DownGua.Yaos[iYao] == 0 || this.DownGua.Yaos[iYao] == 4 ? 4 : 3;
                        }
                        else
                        {
                            this.UpGua.Yaos[iYao % 3] = this.UpGua.Yaos[iYao % 3] == 0 || this.UpGua.Yaos[iYao % 3] == 4 ? 4 : 3;
                        }
            ...
            for (int i = 0; i < 3; i++)
            {
                al.Add(this.DownGua.Yaos[i]);
            }
            for (int i = 0; i < 3; i++)
            {
                al.Add(this.UpGua.Yaos[i]);
            }
            String s = GetGuaFace(al);
        ...
        /// <item><description><b>母宫映射</b>：提取当前复卦所隶属的本宫八纯大卦（<see cref="GuaSelf"/>），并将其完全具象化加载出六亲配置。</description></item>
        ...
        {
            if (this.DownGua.Locs == null) this.DownGua.LoadLocs();
            if (this.UpGua.Locs == null) this.UpGua.LoadLocs();
            if (this.GuaSelf == null) this.LoadGuaSelfandHereThere();
            List<SixRelativeClass> srcs = new List<SixRelativeClass>();
            for (int i = 0; i < 3; i++)
            {
                LocClass lc = this.DownGua.Locs[i];
                FiveAttrRule far = FiveAttr.GetBothAttrRule(this.GuaSelf.FiveAttr.Name, lc.FiveAttr.Name);
                SixRelativeClass src = SixRelativeClass.GetSixRelative(far);
                srcs.Add(src);
            }
            for (int i = 0; i < 3; i++)
        ...
        /// </list>
        /// </remarks>
        public void LoadGuaAliasName()
        {
            bool TorF = false; String sValue = "";
            List<LocClass> lcs1 = this.DownGua.Locs;
            List<LocClass> lcs2 = this.UpGua.Locs;
            for (int i = 0; i < 3; i++)
            {
        ...
        /// </list>
        /// </remarks>
        private void LoadGuaSelfandHereThere()
        {
            if (this.DownGua == null || this.UpGua == null) return;
            int[] Yaos = { 0, 0, 0 };
            Yaos[0] = (this.DownGua.Yaos[0] % 2) + (this.UpGua.Yaos[0] % 2);//下爻
            Yaos[1] = (this.DownGua.Yaos[1] % 2) + (this.UpGua.Yaos[1] % 2);//中爻
            Yaos[2] = (this.DownGua.Yaos[2] % 2) + (this.UpGua.Yaos[2] % 2);//上爻
            for (int i = 0; i < 3; i++)
            {
        ...
        {
            this.SixGods = SixGodClass.GetSixGod(sSkyName).ToList();
        }

        /// <summary>
        /// 内部核心方法（带有递归设计）：根据传入的一字简名或属性模糊名，反向模糊检索并返回 64 卦的四字标准全名。
        /// </summary>
        /// <param name="GuaNameOrAttrName">输入的任意易学名称标识（如单字简名 “乾”、或者属性名 “天” ）。</param>
        /// <returns>返回匹配成功的 64 卦通行本四字全称字符串（如“乾为天”）；若不匹配则返回 <c>null</c>。</returns>
        /// <remarks>
        /// <b>递归终止与检索门道：</b><br/>
        /// <list type="bullet">
        /// <item><description>若输入为单字且属于 <see cref="GuaNames"/>，直接从清册中返回全称（如输入“屯”返回“水雷屯”）。</description></item>
        ...
        private static String GetFullGuaName(String GuaNameOrAttrName)
        {
            ...
            if (sName.Length == 1)
            {
                ...
                if (iPos > -1)
                {
                    sFullName = GuaFullNames[iPos];
                }
                else
                {
                    iPos = Array.IndexOf(GuaSubClass.AfterGuaSubAttrNames, sName);
                    if (iPos > -1)
                    {//属性纯卦
        ...
        /// </summary>
        /// <param name="sGuaName">任意形式的复卦名称（支持单字简名、三字或四字全名）。</param>
        /// <param name="IsUpGua"><c>true</c> 代表期望获取上卦（外卦、悔卦）名称；<c>false</c> 代表期望获取下卦（内卦、贞卦）名称。</param>
        /// <returns>返回对应经卦的单字中文名称（如“乾”、“坤”、“坎”、“离”）；提取失败则返回 <c>null</c>。</returns>
        /// <remarks>
        /// <b>纯卦与杂卦解析门道：</b><br/>
        /// 方法会首先调用 <see cref="GetFullGuaName"/> 将参数统一规范化为四字或三字标准全名。接着将其打散为字符数组：<br/>
        ...
        private static String GetGuaSubName(String sGuaName, bool IsUpGua)
        {
            ...
            sFullName = GetFullGuaName(sGuaName);//转成 卦的全名
            if (sFullName == null) return null;
            if (sFullName.Length >= 3)
            {//三字卦或四字卦
                sd = sGuaName.ToCharArray(); // .toCharArray();

                iPos = Array.IndexOf(GuaSubClass.BeforeGuaSubNames, sd[0].ToString());

                if (iPos == -1)
                {//找不到表示不是纯卦
                    iPos = Array.IndexOf(GuaSubClass.BeforeGuaSubAttrNames, sd[0].ToString());
                    if (iPos == -1) return null;
                    iPos = Array.IndexOf(GuaSubClass.BeforeGuaSubAttrNames, sd[1].ToString());
                    if (iPos == -1) return null;
                    return (IsUpGua ? sd[0].ToString() : sd[1].ToString());//返回上下卦的卦名
                }
                else
                {//找到表示是纯卦
        ...
        /// </summary>
        /// <param name="sGuaNameOrAttr">输入的先天 64 卦简名（如：“乾”、“坤”、“屯”）。</param>
        /// <returns>返回装配好的 <see cref="GuaClass"/> 对象实例。</returns>
        public static GuaClass GetGuaClass(String sGuaNameOrAttr)
        {
            return new GuaClass(sGuaNameOrAttr);
        }

        /// <summary>
        /// 静态工厂方法：依据先天卦序索引创建复卦（六爻卦）对象实例。
        /// </summary>
        /// <param name="iGuaIndex">先天 64 卦的原始数组索引（取值范围：0 至 63）。</param>
        /// <returns>返回装配好的 <see cref="GuaClass"/> 对象实例。</returns>
        public static GuaClass GetGuaClass(int iGuaIndex)
        {
            return new GuaClass(iGuaIndex);
        }
        ...
        {
            return GetGuaClass(iYaos.ToList());
        }
        ...
        public static GuaClass GetGuaClass(List<int> iYaos)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (iYaos[i] == 3 || iYaos[i] == 4)//3为阳爻动，4为阴爻动
                {
                    ...
                    int iMod = i % 3;
                    if (i < 3) gc.DownGua.Yaos[iMod] = iYaos[i]; else gc.UpGua.Yaos[iMod] = iYaos[i];
                }
            }
            return gc;
        }
        ...
        /// <returns>返回依照先天梅花数理生成的 <see cref="GuaClass"/> 复卦实例。</returns>
        /// <remarks>
        ...
        /// </remarks>
        public static GuaClass FlowerGuaClass(int iv1, int iv2)
        {
            ...
            GuaClass gc = GetGuaClass(sName);
            int iv;
            if (iYaoDoing < 3)
            { //则下卦动
                iv = gc.DownGua.Yaos[iYaoDoing % 3];
                iv = iv == 0 ? 4 : 3;
                gc.DownGua.Yaos[iYaoDoing % 3] = iv;
            }
            else
            {
                iv = gc.UpGua.Yaos[iYaoDoing % 3];
                iv = iv == 0 ? 4 : 3;
                gc.UpGua.Yaos[iYaoDoing % 3] = iv;
            }
            List<int> al = new List<int>();
