...
namespace CompassEx.Data.Models {
	...
	/// <summary>
	/// 一键转换为支持 WPF 双向绑定的 tbl_YearVM 对象
	/// </summary>
	public virtual tbl_YearVM ToViewModel() {
		return new tbl_YearVM {
			YearID = this.YearID,
		...
		private static IFreeSql SafeOrm => Orm ?? throw new Exception($"{nameof(tbl_Year)}.Orm 未在程序启动时初始化。");
		...
		public void Refresh()
		{
			var dbEntity = One(t => t.YearID == this.YearID);
			if (dbEntity != null)
			{
				foreach (var prop in typeof(tbl_Year).GetProperties(BindingFlags.Public | BindingFlags.Instance))
				{
					if (prop.CanWrite)
					{
		...
		/// </summary>
		public static ISelect<tbl_Year> Select => SafeOrm.Select<tbl_Year>();
		
		/// <summary>
		/// 🎯 PageList 静态方法：纯粹获取分好页的数据集合（不含任何 out 属性），pageIndex 小于等于 0 时自动查全部
		/// </summary>
		public static List<tbl_Year> List(System.Linq.Expressions.Expression<Func<tbl_Year, bool>> exp = null, int pageIndex = 0, int pageSize = 10, string sortField = null, bool isDesc = true)
		{
			var query = Select;
		...
		/// </summary>
		public static long GetRecordCount(System.Linq.Expressions.Expression<Func<tbl_Year, bool>> exp = null)
		{
			var query = Select;
			if (exp != null) query = query.Where(exp);
		...
		/// </summary>
		public static tbl_Year One(System.Linq.Expressions.Expression<Func<tbl_Year, bool>> exp= null, string sortField = null, bool isDesc = true)
		{
    ...
    /// </summary>
    public partial class tbl_YearVM : tbl_Year, INotifyPropertyChanged {
