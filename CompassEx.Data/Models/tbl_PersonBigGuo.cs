...
namespace CompassEx.Data.Models {
	...
	public partial class tbl_PersonBigGuo {
		...
		[JsonProperty, Column(DbType = "TEXT(50)", IsNullable = false)]
	...
	/// <summary>
	/// 一键转换为支持 WPF 双向绑定的 tbl_PersonBigGuoVM 对象
	/// </summary>
	public virtual tbl_PersonBigGuoVM ToViewModel() {
		return new tbl_PersonBigGuoVM {
		...
		private static IFreeSql SafeOrm => Orm ?? throw new Exception($"{nameof(tbl_PersonBigGuo)}.Orm 未在程序启动时初始化。");
		...
		public void Refresh()
		{
			...
			if (dbEntity != null)
			{
				foreach (var prop in typeof(tbl_PersonBigGuo).GetProperties(BindingFlags.Public | BindingFlags.Instance))
				{
					if (prop.CanWrite)
					{
		...
		/// </summary>
		public static ISelect<tbl_PersonBigGuo> Select => SafeOrm.Select<tbl_PersonBigGuo>();
		
		/// <summary>
		/// 🎯 PageList 静态方法：纯粹获取分好页的数据集合（不含任何 out 属性），pageIndex 小于等于 0 时自动查全部
		/// </summary>
		public static List<tbl_PersonBigGuo> List(System.Linq.Expressions.Expression<Func<tbl_PersonBigGuo, bool>> exp = null, int pageIndex = 0, int pageSize = 10, string sortField = "CreateTime", bool isDesc = true)
		{
			var query = Select;
		...
		/// </summary>
		public static long GetRecordCount(System.Linq.Expressions.Expression<Func<tbl_PersonBigGuo, bool>> exp = null)
		{
			var query = Select;
			if (exp != null) query = query.Where(exp);
		...
		/// </summary>
		public static tbl_PersonBigGuo One(System.Linq.Expressions.Expression<Func<tbl_PersonBigGuo, bool>> exp= null, string sortField = "CreateTime", bool isDesc = true)
		{
    ...
    /// </summary>
    public partial class tbl_PersonBigGuoVM : tbl_PersonBigGuo, INotifyPropertyChanged {
